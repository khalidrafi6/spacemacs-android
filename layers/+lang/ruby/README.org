...
| ~SPC m R r b~ | Runs RuboCop on the currently visited file           |
| ~SPC m R r B~ | Runs auto-correct on the currently visited file      |
...
| ~SPC m t l~ | repeat last test command    |
| ~SPC m t t~ | run test around point       |
